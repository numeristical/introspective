...
def test_opt_err_func():
    ...
    my_y = my_x
    my_err = mli.opt_err_func(my_params, my_x, my_y, my_silly_func)
    ...
    my_err = mli.opt_err_func(my_params, my_x, my_y, not_so_silly_func)
    # Since x and y are equal, the error is zero:
    npt.assert_equal(my_err, np.zeros(my_x.shape[0]))
...
def test_Model():
    ...
    target_mu = 0.5
    target_sigma = 1
    target_y = mli.cumgauss(x, target_mu, target_sigma)
